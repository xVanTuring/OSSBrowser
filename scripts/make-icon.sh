...
# (deduplicated layout — 7 PNGs each reused across multiple scale
# slots in Contents.json).
#
# Run after any edit to design/AppIcon.svg. The output PNGs and
# Contents.json are committed so the build doesn't need rsvg-convert;
...
ASSETS="OSSBrowser/Assets.xcassets"
...
# One PNG per pixel size, mapped to multiple scale slots in
